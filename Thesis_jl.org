...
function L(solver::relaxed_multi_solver,i,j , phi , mu)
    xi = solver.phase[i, j] / solver.dt -
         (discrete_G_weigted_neigbour_sum(i, j, solver.potential, G, solver.len, solver.width)
          -
          neighbours_in_domain(i, j, G, solver.len, solver.width) * mu )/solver.h^2
    psi = solver.epsilon^2/solver.h^2 * solver.alpha
          (solver.c[I] - phi) - 2 * phi + mu
    return [xi, psi]
end
#+end_src
*** SMOOTH
...
\end{align*}
**** Proposal2
solve  for \( \overline{\mu_{ij}^{n+1,m}}  \)  and \( (\overline{\phi_{ij}^{n+1,m}})^{\alpha} \). This was not done in the original paper as the there required System of linear equations was solved numerically. The relaxation simplifies the it to one dimension, and enables exact solutions:
...
function SMOOTH_relaxed!(
    ...
    for k = 1:iterations
        ...
        for I in CartesianIndices(solver.phase)[2:end-1, 2:end-1]
            ...
            solver.phase[I] = (solver.epsilon^2 * solver.alpha * solver.c[I] - solver.h^2 / bordernumber * ( -1*solver.xi[I]  - discrete_G_weigted_neigbour_sum(i,j,solver.potential , G , solver.len , solver.width) / solver.h^2 ) - solver.psi[I]) / (solver.epsilon^2 * solver.alpha  + 2 + solver.h^2 / (bordernumber*solver.dt))

            solver.potential[I] = (solver.phase[I]/solver.dt - solver.h^-2 * discrete_weigted_neighbour_sum(i,j, solver.potential[I] , G , solver.len , solver.width)) / (solver.h^2/bordernumber)
        end
...
#+begin_src julia-vterm :file smooth_relaxed.svg :output-dir images :results file graphics :noweb yes
using Plots
using LaTeXStrings
using LinearAlgebra
include(pwd() * "/"*"/utils.jl")
...
set_xi_and_psi!(solver)
p1 = heatmap(solver.phase, aspect_ratio=:equal, title="without solving c" , xlim=(2,SIZE) , ylim=(2,SIZE));
<<elyps_solver>>
solver = relaxed_multi_solver(
    phase ,
    zeros(size(phase)) ,
...
SMOOTH_relaxed!(solver, 200, true);
...
*** Relaxed Equation Operator L
*** Relaxed V-cycle
#+begin_src julia :tangle multi_relaxed.jl :eval never
function relaxed_v_cycle(grid::Array{multi_solver}, level)
