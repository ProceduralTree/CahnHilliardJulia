...
\end{cases}
\end{align*}
where \( \Omega_{d} \) is the discrete version or our domain.

#+begin_src julia-vterm :results file graphics :file domain.svg :post "#+caption discrete domain"
using Plots
using LaTeXStrings
...
#+RESULTS[b75a46b7be7e7858f8ecc9eea86b43923a142ddc]:
[[file:images/domain.svg]]

the domain we calculate on is therefore a square starting at \( (2,2) \) and ending at \( (N+1,N+1) \). We do this to accommodate for zero padding in our numerical implementation.
...
#+name: fig:relaxed-enrgy-balance
...
#+RESULTS: fig:relaxed-enrgy-balance
