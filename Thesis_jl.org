...
function v_cycle(grid::Array{multi_solver}, level)
    ...
    for i = 1:5
        for I in CartesianIndices(solver.phase)[2:end-1, 2:end-1]
            solution.phase[I], solution.potential[I] = L(solution, I.I...) .+ ((solver.phase[I], solver.potential[I]) .- ((d_large[I], r_large[I]) .+ L(solver, I.I...))) .\ dL(solver, I.I...)
        end
    end
    print("Finished  largegrid")
    ...
    solver.phase[2:end-1, 2:end-1] += prolong(u_large[2:end-1, 2:end-1], (solver.len, solver.width))
    solver.potential[2:end-1, 2:end-1] += prolong(v_large[2:end-1, 2:end-1], (solver.len, solver.width))
    # smooth again:
    SMOOTH!(solver, 800, true)
    print("Finished post SMOOTHing")
end
#+end_src
