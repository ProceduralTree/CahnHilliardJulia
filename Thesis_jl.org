...
#+name: inputs from [[Initial data]] after SMOOTH
...
3. a Newton iteration to solve \( L(x,y)_H = L(\bar{x} , \bar{y}) + (d_h , r_h) \).
   we solve using the same iteration as in [[SMOOTH operator]] however we replace \( (\zeta , \psi) \) with \( L(\bar{x} , \bar{y}) + (d_h , r_h) \) in the iteration.
...
p1 = plot(1:64 , energy , title=L"Discrete Helmholtz Energy $E_d^{bulk}$", xlabel="timesteps" , ylabel="energy" )
...
Furthermore, experimentation shows that alpha allone is insuficcient to get a relaxed method consistent with the original solver, since alpha had an effect simmilar to epsilon, where it changed the boundary thickness in the phase-field \( \phi \). Therefore epsilon and alpha cannot be chosen indepentantly. Hence we use a simple MCMC optimizer for \( \alpha,\varepsilon \) in order to give the relaxed solver the best chance we can.
...
using Measures
i0 = 64 * 1 +1
var"W_prime#20"(x) = -x * (1 - x^2)
results = jldopen("experiments/relaxed_iteration.jld2")["result"]
