...
p1 = plot(1:64 , energy .- energy[1], xlabel= "time-steps" , ylabel = "error"  , label ="balance" , clim=(-1,1))
...
p= res.cycle_function[7*64:8*64-2] |> (x)-> plot(x , yscale=:log10 , title="Behaviour of the solver for increasing sub-iterations" , xlabel="sub-iterations" , ylabel= "diffrence" , label= L" \|\phi^{n+1,m} - \phi^{n+1,m-1}\|_{Fr} ")
...
#+begin_src julia-vterm :results file graphics :file relaxed-energy-balance.svg :output-dir images :noweb no-export :session jl
<<init>>
using JLD2
using DataFrames
default(fontfamily="computer modern" , titlefontsize=32 , guidefontsize=32 )
