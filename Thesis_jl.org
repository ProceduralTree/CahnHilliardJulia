...
since values for \( \phi_{kl}^{n+1,m} , \mu_{kl}^{n+\frac{1}{2},m} \) are unknown the authors in [cite:@SHIN20117441] and we use initial approximations  if \( k > i , l > j \) , and the values of the current smooth iteration else. As initial approximation we use the values of \(  \phi_{kl}^{n+1,m} , \mu_{kl}^{n+\frac{1}{2},m}  \) in the last smoothing iteration.
...
tests = [testgrid(multi_solver , M , 2 , dt = t ) for t in 1e-2./(1:128)]
...
p= res.cycle_function[7*64:8*64-2] |> (x)-> plot(x , yscale=:log10 , title="Beaviour of the solver for increasing sub-iterations" , xlabel="amount of sub-iterations" , ylabel= "diffrence to previous sub-iteration" , label= L" \|\phi^{n+1,m} - \phi^{n+1,m-1}\|_{Fr} ")
