...
1. a Gauss-Seidel relaxation for smoothing on the fine grid \( h \), as described in Chapter [[SMOOTH operator]].
...
4. a Gauss-Seidel iteration to solve \( L(\hat{\phi}_{ij,H}^{n+1,m}, \hat{\mu}_{ij,H}^{n+\frac{1}{2},m})_H = L(\bar{\phi}_{ij,H}^{n+1,m} , \bar{\mu}_{ij,H}^{n+\frac{1}{2},m}) + (d_{ij,H}^{n+1,m} , r_{ij,H}^{n+1,m}) \).
...
function alt_v_cycle!(grid::Array{T}, level) where T <: solver
    ...
    for I in CartesianIndices(solver.phase)[2:end-1, 2:end-1]
        d[I], r[I] = [solver.xi[I], solver.psi[I]]
...
#+RESULTS[67b472ddafeb65ec4005a1bbc9c3220b10030ad8]: fig:stability-in-space
...
using JLD2
using LaTeXStrings
default(fontfamily="computer modern" , titlefontsize=32 , guidefontsize=18 , tickfontsize = 12 , legendfontsize=18)
gr()
...
#+RESULTS[2d1d59898b534f074ded1c9b41ae4828abeeea80]: fig:relaxed-stability-in-space
[[file:images/relaxed-space-stability.svg]]

* Comparison
** effect of alpha
In the previous chapter we have shown the stability, and lack thereof, for the relaxed solver. However in this chapter we show a direct comparison between both methods. Since the relaxed solver is dependant on the relaxation variable \( \alpha \).
...
#+end_src
#+begin_src julia    :noweb no-export :tangle experiments/src/alt-relaxed-iteration.jl :async
using JLD2
using DataFrames
using ProgressMeter
using Random
...
jldsave("experiments/alt-relaxed-iteration-less-smooth.jld2"; result)
#+end_src
...
*** mass
#+begin_src julia-vterm :results file graphics :file alt-relaxed-mass-balance.svg
<<init>>
using JLD2
using DataFrames
using Measures
...
i0 = 64 * 1+1
results = jldopen("experiments/alt-relaxed-iteration-less-smooth.jld2")["result"]
energy = [ massbal(s.phase) .- massbal(results.solver[i0].phase) for s in results[i0:i0+63,:].solver]
p1 = plot(1:64 , energy, xlabel= "time-steps" , ylabel = "error"  , label =false)
p2 = heatmap(results.solver[i0].phase , title="initial condition" , legend=:none , aspectratio=:equal , grid=false , showaxis=false)
p3 = heatmap(results.solver[i0+63].phase , title="after 64 time-steps" , legend=:none , aspectratio=:equal , grid=false , showaxis=false)
p = plot(p2,p3 , p1 , layout=layout3x1 , size=(1600 ,1600))
savefig(p , "images/alt-relaxed-mass-balance.svg")
#+end_src

#+RESULTS[0c2d32d827d6b633e5934778ba800d45d35c5d45]:
[[file:images/alt-relaxed-mass-balance.svg]]
