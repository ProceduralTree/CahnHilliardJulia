...
\begin{aligned}
 \varepsilon ^2 \alpha \phi ^{n+1,m,\alpha}_{ij} + 2 \phi ^{n+1,m,\alpha}_{ij} &= \varepsilon ^2 \alpha c^{n,\alpha}_{ij}  -\mu^{n + \frac{1}{2},m,\alpha}_{ji}  - \psi_{ij}^{n,\alpha}  \\
\phi_{ij}^{n+1,m,\alpha} &= (\varepsilon^2\alpha + 2)^{-1} \left( \varepsilon^2\alpha c^{n+1,\alpha}_{ij} - \mu^{n+\frac{1}{2},m,\alpha}_{ij} - \psi_{ij}^{n,\alpha} \right)
...
function SMOOTH!(
    ...
    for k = 1:iterations
        ...
        for I in CartesianIndices(solver.phase)[2:end-1, 2:end-1]
            ...
            solver.phase[I]  = (solver.epsilon^2 * solver.alpha * solver.c[I] - solver.potential[I] - solver.psi[I])/(solver.epsilon^2 * solver.alpha +2)
