...
function SMOOTH!(
    ...
    for k = 1:iterations
        ...
        for I in CartesianIndices(solver.phase)[2:end-1, 2:end-1]
            i, j = I.I
            bordernumber = neighbours_in_domain(i, j, G, solver.len, solver.width)
            solver.potential[I] = (solver.phase[I]/solver.dt - solver.xi[I]
                                   - discrete_G_weigted_neigbour_sum(i,j, solver.potential , G , solver.len , solver.width)/solver.h^2) * (-solver.h^2/bordernumber)

            solver.phase[I]  = 1/(solver.epsilon^2 * solver.alpha +2) * (solver.epsilon^2 * solver.alpha * solver.c[I] - solver.potential[I] - solver.psi[I])
