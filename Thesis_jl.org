...
We chose our Domain \( \Omega \) to be a cartesian Grid on a square with sidelength \( Nh \), where N is the Number of gridpoints in one Direction, and \( h \) is the distance between gridpoints. In for all our initial Data \( h \) is \( 3\cdot10^{-3}\).
We discretize the phase-field ,\( \phi \), and chemical potential ,\( \mu \), into grid-wise functions \(\phi_{ij}, \mu_{ij} \), where \( \cdot_{ij} \) represents the evaluation of \( \cdot \) at index \( ij \) . and at coordinates \( (i\cdot h , j\cdot h) \).

Defines the partial derivatives \( D_x\phi_{ij}, \ D_y\phi_{ij} \) using the differential quotients:
\begin{align}
D_x\phi_{i+\frac{1}{2} j} &= \frac{\phi_{i+1j} - \phi_{ij}}{h} & D_y\phi_{ij+\frac{1}{2}} &= \frac{\phi_{ij+1} - \phi_{ij}}{h}
\end{align}
For \( \nabla_d \phi, \nabla_d \cdot (G_{ij} \nabla_d \phi )\) then follows:
