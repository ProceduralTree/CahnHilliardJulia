...
\end{aligned}
\end{equation}
it therefore holds for a relaxed enregy:
\begin{equation}
...
And in fact, when observing the discertization on the square domain \( \Omega_d \) we are able to show the following:
\begin{equation}
\begin{aligned}
\sum_{i=2}^{N+1} \sum_{j=2}^{N+1} \frac{1}{\Delta t} \left( \phi_{ij}^{n+1} - \phi_{ij}^{n} \right) &= \sum_{i=2}^{N+1}\sum_{j=2}^{N+1} \nabla_d \cdot (G_{ij} \nabla_d \mu_{ij}^{n+\frac{1}{2}}) \\
\end{aligned}
\end{equation}
We split the right double sum into three parts. We consider them seperately. The first part consists of the inner sum, where \( G_{i+\frac{1}{2}j} = G_{i+\frac{1}{2}j} = G_{ij+\frac{1}{2}} = G_{ij-\frac{1}{2}} = 0 \). The inner sum can therefore be written as such:
\begin{equation}
\begin{aligned}
...
\sum_{j=3}^{N} \frac{\Sigma_G\mu^{\alpha}_{2j}^{n+\frac{1}{2}} - \Sigma_{G2j}\cdot \mu_{2j}^{n+\frac{1}{2}}}{h^2} &= (\mu_{2,3}+\mu_{2,N})- (\mu_{2,N+1}+\mu_{2,2}) - \sum_{j=3}^{N} \mu^{n+\frac{1}{2}}_{2j} - \mu^{n+\frac{1}{2}}_{3j} \\
...
\begin{equation}
\mu_{N+1,N}+\mu_{N+1,3} - 2\mu_{N+1,N+1} \\
\mu_{2,3}+\mu_{2,N} - 2\mu_{2,2}\\
\mu_{N,2}+\mu_{N+1,3} - 2\mu_{2,N+1}\\
\mu_{N,N+1}+\mu_{3,N+1} - 2\mu_{N+1,2}\\
\end{equation}
