...
As baseline for further experiments a two grid method based on finite differences by[cite:@SHIN20117441]. Is used.
** Discretization:
...
#+end_src
this operator follows from [[eq:discrete-cahn-hilliard]] by separating implicit and explicit terms \( L \) an d   \( (\zeta^n_{ij} , \psi^n_{ij})^T \) respectivly.
\begin{align*}
\begin{pmatrix}
\zeta^n
 \\
