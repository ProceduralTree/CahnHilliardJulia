...
The paper [cite:@Wu_2022] states that the observable phase seperation is driven by a diffusion resulting from the gradient in chemical potential \( \mu \). The emergent conservative dynamics motivate the following diffusion equation
#+name: eq:massbal
\begin{equation}
...
To show the conservation of mass we analyse the change in total mass in the domain \( \Omega \) over time.
