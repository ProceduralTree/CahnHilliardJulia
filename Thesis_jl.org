...
plot!(p, i_0:n, change[i_0:end],  title = L"$\| \phi^{n+1,m-1} - \phi^{n+1,m}  \|$", yaxis=:log,label=L"t=%$t \Delta t")
end
savefig(p , "images/time-convergence.svg")
...
#+begin_src julia-vterm :noweb yes
<<setup-grid>>
using JLD2
using Dataframes
df = jldload("data/train_data.jld2")["result"]
df[!,"grad"] = ggrad.(df[!,"expected"] , Ref(test_solver))
#+end_src

#+RESULTS:
: LoadError: syntax: incomplete: "function" at /home/jon/Projects/julia_tst/src/utils.jl:87 requires end
: in expression starting at /home/jon/Projects/julia_tst/src/utils.jl:87
: in expression starting at /tmp/babel-xiyd0R/julia-vterm-src-t7P7Hc:1
